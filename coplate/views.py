...
class ProcessLikeView(LoginAndVerificationRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        like, created = Like.objects.get_or_create(
            user=self.request.user,
            content_type_id = self.kwargs.get('content_type_id'), # url에 있는 <int:content_type_id>를 가져옴
            object_id =self.kwargs.get('object_id'),     # url에 있는 <int:object_id>를 가져옴
        )
        if not created :
            like.delete()
